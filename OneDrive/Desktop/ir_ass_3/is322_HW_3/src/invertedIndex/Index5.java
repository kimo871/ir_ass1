...
public class Index5 {
    ...
    public void  buildQueryDocument(String ans){
        String[] words = ans.split("\\W+");
    ...
    public int buildIndex(String ln, int fid) {
        ...
        for (String word : words) {
            ...
            if( i<words.length-1){
                if(words[i+1] != words[i])sources.get(fid).norm += Math.pow(index.get(word).last.dtf,2);
            }
            else if(i == words.length-1)sources.get(fid).norm += Math.pow(index.get(word).last.dtf,2);
    ...
    //-----------------------------------------------
    public void buildIndex(ArrayList<String> files) {  // from disk not from the internet
        int fid = 0;
...
//==========================================================    
   public String find_07a(String phrase) {
        ...
        buildQueryDocument(phrase); // representing query as document to be used in similarity
        String result = "";
        String[] words = phrase.split("\\W+");
        int len = words.length;
        sortedScore = new SortedScore();
        System.out.println("sources size" + " "+sources.size());
        double scores[] = new double[sources.size()];
        double cosineSim[] = new double[sources.size()] ;
...
//3 for each query term t
      for(String term : words){
         ...
         double tdf = index.get(term)==null ? 0 : index.get(term).doc_freq;
         double ttf = index.get(term)==null ? 0 : index.get(term).term_freq;
         double idf = tdf == 0 ? 0 : log10(sources.size()/(double)tdf);
         Posting pList = index.get(term)==null ? null : index.get(term).pList;
         //System.out.println(index.get(term).pList.docId);
         while(pList!=null){
        ...
        // System.out.println("product dot "+cosineSim[0]);
      ...
      for(int y=0; y<sources.size();y++){
        sources.get(y).norm = Math.sqrt(sources.get(y).norm);
        cosineSim[y] /= (sources.get(y).norm*queryNorm);
        System.out.println(sources.get(y).PrintRecord());
        sortedScore.insertScoreRecord(new ScoreRecord(cosineSim[sources.get(y).fid], sources.get(y).title, sources.get(y).URL));
      }
      sortedScore.printScores();

      //fid=f; URL=u; title=tt; text=tx;
      // Normalize scores for document length
